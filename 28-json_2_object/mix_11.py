    ...
    if isinstance(data, list):  # list of dictionaries that' represent the model
        ...
        for k, v in data.items():
            if not isinstance(v, dict) and not isinstance(v, list):
                ...
                try:
                    attr = getattr(obj,
                                   k)  # The list needs to have at least 1 item in the collection to be able to verify object type
        ...
        raise Exception(
            'Data representation of model needs to be in a list of dictionaries, dictionary or str json. Check to see if format is correct.')